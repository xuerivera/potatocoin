...
function validateSort(data, state2, x, y) {
  ...
  if (state2.draggedNodes.map((x2) => x2.el).includes(data.targetData.node.el))
    return false;
  if (state2.preventEnter || state2.swappedNodeValue === data.targetData.node.data.value || data.targetData.parent.el !== ((_a = state2.lastParent) == null ? void 0 : _a.el) || data.targetData.parent.data.config.sortable === false)
    return false;
  const targetRect = data.targetData.node.el.getBoundingClientRect();
